            ...
            try
            {
                ...
                using (SqlConnection sqlConnection = new SqlConnection(ADOpet.connectionString))
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        int id = Convert.ToInt32(reader["Id"]);
                        string name = (string)reader["Nombre"];
                        string clinicHistory = reader["HistoriaClinica"] == DBNull.Value ? null : (string)reader["HistoriaClinica"];
                        DateTime birth = Convert.ToDateTime(reader["Nacimiento"]);
                        int dni = Convert.ToInt32(reader["Dni"]);
                        int phone = Convert.ToInt32(reader["Celular"]);
                        ...
                        list.Add( new Pet(id, name, tipo, birth, dni, phone, clinicHistory) );
                    }
        ...
        public static void Update(Pet pet)
        {
            try
            {
                string query = $"UPDATE Pet "+
                    "SET Nombre=@Nombre,HistoriaClinica=@HistoriaClinica,Nacimiento=@Nacimiento,Tipo=@Tipo,Dni=@Dni,Celular=@Celular WHERE Id=@Id";
